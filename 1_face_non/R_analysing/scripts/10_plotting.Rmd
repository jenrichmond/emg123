...
### Average muscle activity during 1000ms (averaging across bin) - FIgure 4
...
### CHEEK - Changes in Facial muscle activity over time - Figure 5
```{r}
...
### XX BROW - avg.d across stimulus type - Figure XX

```{r}
face_brow_bin <- face_screened %>%
  filter(muscle == "brow") %>%
  group_by(bin) %>%
  summarise(meanZ = mean(Zdiff, na.rm = TRUE), 
            sd_RMS = sd(Zdiff, na.rm = TRUE), 
  ...
  scale_x_discrete(labels = c("bin1" = "Bin 1", "bin2" = "Bin 2", "bin3" = "Bin 3", "bin4" = "Bin 4", "bin5" = "Bin 5", "bin6" = "Bin 6", "bin7" = "Bin 7", "bin8" = "Bin 8", "bin9" = "Bin 9", "in10" = "Bin 10")) +
  xlab("Bin") + 
  ylab("Mean z-score differences in brow responding \n (averaged across facial stimuli)")

print(face_brow_bin)

ggsave("face_brow_bin_plot.png", path = here("R_analysing", "plots")) 

```

### BROW - responding over time - Figure 6
```{r}
...
### Activity avg.d across bin - Cheek and Brow - Figure 7 
...
### CHEEK - activity averaged across stimulus type - Figure 8
```{r}
non_cheek_bin <- non_screened %>%
  filter(muscle == "cheek") %>%
  group_by(bin) %>%
  summarise(meanZ = mean(Zdiff, na.rm = TRUE), 
            sd_RMS = sd(Zdiff, na.rm = TRUE), 
...
ggsave("non_cheek_bin_plot.png", path = here("R_analysing", "plots")) 

```


### CHEEK - responding over time - Figure 9
```{r}
...
## BROW - activity across time - Figure 10
```{r}
