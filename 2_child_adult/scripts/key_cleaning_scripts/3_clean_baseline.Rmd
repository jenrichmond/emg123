...
```{r}
clean_data <- read_csv(here("rileyraw_data", "adult_child_combined", "child_adult_clean.csv"))
...
# add mean and sd columns and true false if bl_rms is > 1.5sd above the mean for the participant, emotion, muscle
```{r}
...
#count how many trials have bad baselien
...
# drop extra variables, make bl_outlier as.character and bind stim and BL back together
...
no_bl_outliers %>% 
  write_csv(here("rileyraw_data", "adult_child_combined", "clean_no_bl_outliers.csv"))
```
...
just_bl_outliers %>%
  write_csv(here("rileyraw_data", "adult_child_combined", "clean_just_bl_outliers.csv"))
